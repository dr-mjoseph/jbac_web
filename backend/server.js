...
app.get(['/api/crud/:table', '/dashboardapi/crud/:table', '/crud/:table'], async (req, res) => {
    ...
    try {
        ...
        const [countResult] = await db.query(`SELECT COUNT(*) as total FROM \`${table}\` ${whereClause}`, queryParams.slice(0, filters.length));
...
app.post(['/api/crud/:table', '/dashboardapi/crud/:table', '/crud/:table'], async (req, res) => {
    const table = req.params.table;
    if (!isValidTable(table)) {
        return res.status(400).json({ status: 400, error: `Invalid table: ${table}` });
    }
...
// Update record by ID
app.put(['/api/crud/:table/:id', '/dashboardapi/crud/:table/:id', '/crud/:table/:id'], async (req, res) => {
    const table = req.params.table;
    const id = req.params.id;
    if (!isValidTable(table)) {
        return res.status(400).json({ status: 400, error: `Invalid table: ${table}` });
...
// Delete record by ID
app.delete(['/api/crud/:table/:id', '/dashboardapi/crud/:table/:id', '/crud/:table/:id'], async (req, res) => {
    const table = req.params.table;
    const id = req.params.id;
    if (!isValidTable(table)) {
        return res.status(400).json({ status: 400, error: `Invalid table: ${table}` });
...
app.all(['/api/events', '/dashboardapi/getevents', '/dashboardapi/getupdateevents', '/api/getupdateevents'], async (_req, res) => {
    try {
        const [eventsRows] = await db.query('SELECT id, title, COALESCE(event_name, title) as event_name, description, event_date, event_time, location, image FROM events ORDER BY id DESC');
        if (eventsRows && eventsRows.length > 0) {
            return res.json({ status: 200, data: eventsRows });
        }
        const [annRows] = await db.query('SELECT * FROM announcements ORDER BY id DESC');
...
app.post(['/api/register-all', '/dashboardapi/postregform', '/api/register-member'], async (req, res) => {
    try {
        ...
        const [regResult] = await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', [type, first, last, userEmail, userPhone, JSON.stringify(extra)]);
        try {
            await db.query('INSERT INTO members (first_name, last_name, email, phone) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE first_name=VALUES(first_name)', [first, last, userEmail, userPhone]);
        }
        catch (_) { }
        res.status(201).json({
            status: 200,
...
app.post(['/dashboardapi/postbeliversignup', '/api/postbeliver', '/dashboardapi/postbeliver'], async (req, res) => {
    try {
        ...
        if (phone) {
            const [existing] = await db.query('SELECT id FROM believers WHERE mobile_number = ?', [phone]);
            if (existing && existing.length > 0) {
                return res.json({ status: 451, message: 'Mobile number already registered.' });
            }
        }
        const [cols] = await db.query('DESCRIBE believers');
        ...
        await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', ['believer', b.fname || 'Believer', b.lname || '', b.email || null, phone || null, JSON.stringify(b)]);
        if (phone && b.password) {
            await db.query('INSERT INTO users (name, mobile_number, email, password, category) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE password=VALUES(password)', [`${b.fname || ''} ${b.lname || ''}`.trim() || 'Believer', phone, b.email || null, b.password, 1]);
        }
...
app.post(['/dashboardapi/postpastor', '/api/postpastor', '/dashboardapi/postrpastor'], async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', ['pastor', name, '', p.email || null, phone || null, JSON.stringify(p)]);
        await db.query('INSERT INTO pastors (name, mobile_number, email, password, qualification, denomination, church_name, district, constituency, mandal, village, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, phone || '', p.email || null, p.password || '', p.qualification || '', p.denomination || '', p.church_name || '', p.district || '', p.constituency || '', p.mandal || '', p.village || '', p.address || '']);
        if (phone && p.password) {
            await db.query('INSERT INTO users (name, mobile_number, email, password, category) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE password=VALUES(password)', [name, phone, p.email || null, p.password, 4]);
        }
...
app.post(['/dashboardapi/postchurchregister', '/api/postchurchregister'], async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', ['church', name, c.pastor_name || '', c.email || null, phone || null, JSON.stringify(c)]);
        await db.query('INSERT INTO churches (church_name, pastor_name, mobile_number, email, password, denomination, district, constituency, mandal, village, address, registration_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, c.pastor_name || '', phone || '', c.email || null, c.password || '', c.denomination || '', c.district || '', c.constituency || '', c.mandal || '', c.village || '', c.address || '', c.registration_number || '']);
        if (phone && c.password) {
            await db.query('INSERT INTO users (name, mobile_number, email, password, category) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE password=VALUES(password)', [name, phone, c.email || null, c.password, 5]);
        }
...
app.post(['/dashboardapi/postindepedentorganisation', '/api/postindepedentorganisation'], async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', ['organisation', name, o.leader_name || '', o.email || null, phone || null, JSON.stringify(o)]);
        await db.query('INSERT INTO organisations (org_name, leader_name, mobile_number, email, password, category, district, constituency, mandal, village, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, o.leader_name || '', phone || '', o.email || null, o.password || '', o.category || '', o.district || '', o.constituency || '', o.mandal || '', o.village || '', o.address || '']);
        if (phone && o.password) {
            await db.query('INSERT INTO users (name, mobile_number, email, password, category) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE password=VALUES(password)', [name, phone, o.email || null, o.password, 6]);
        }
...
app.post(['/dashboardapi/postministrysignup', '/api/postministrysignup'], async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', ['ministry', name, m.leader_name || '', m.email || null, phone || null, JSON.stringify(m)]);
        await db.query('INSERT INTO ministries (ministry_name, leader_name, mobile_number, email, password, denomination, district, constituency, mandal, village, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, m.leader_name || '', phone || '', m.email || null, m.password || '', m.denomination || '', m.district || '', m.constituency || '', m.mandal || '', m.village || '', m.address || '']);
        if (phone && m.password) {
            await db.query('INSERT INTO users (name, mobile_number, email, password, category) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE password=VALUES(password)', [name, phone, m.email || null, m.password, 3]);
        }
...
app.post(['/dashboardapi/studentsignup', '/api/studentsignup'], async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', ['student', name, '', s.email || null, phone || null, JSON.stringify(s)]);
        await db.query('INSERT INTO students (student_name, mobile_number, email, password, course, college_name, district, constituency, mandal, village, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, phone || '', s.email || null, s.password || '', s.course || '', s.college_name || '', s.district || '', s.constituency || '', s.mandal || '', s.village || '', s.address || '']);
        if (phone && s.password) {
            await db.query('INSERT INTO users (name, mobile_number, email, password, category) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE password=VALUES(password)', [name, phone, s.email || null, s.password, 2]);
        }
...
app.post(['/dashboardapi/postpastorassociations', '/api/postpastorassociations'], async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', ['pastorassociation', name, a.leader_name || '', a.email || null, phone || null, JSON.stringify(a)]);
        await db.query('INSERT INTO pastor_associations (association_name, leader_name, mobile_number, email, password, district, constituency, mandal, village, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, a.leader_name || '', phone || '', a.email || null, a.password || '', a.district || '', a.constituency || '', a.mandal || '', a.village || '', a.address || '']);
        if (phone && a.password) {
            await db.query('INSERT INTO users (name, mobile_number, email, password, category) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE password=VALUES(password)', [name, phone, a.email || null, a.password, 7]);
        }
...
app.all(['/dashboardapi/getatt', '/api/getatt'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM attacks ORDER BY id DESC');
...
app.post(['/dashboardapi/postjobs', '/api/postjobs'], async (req, res) => {
    try {
        const j = req.body;
        await db.query(`INSERT INTO jobs (jobname, qualification, experience, salary, location, description, number1, number2, districtname, constituencyname, mandals, village_name, google_location, image, facebook, youtube, usr_id, name, mobile_number)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
...
app.all(['/dashboardapi/getjobs', '/dashboardapi/searchjob', '/dashboardapi/getjob', '/api/getjobs'], async (req, res) => {
    try {
        ...
        const wheres = [];
        if (district_id) {
            wheres.push('(districtname = ? OR id = ?)');
            params.push(district_id, district_id);
        }
        if (constenncy_id) {
            wheres.push('(constituencyname = ?)');
            params.push(constenncy_id);
        }
        if (mandal_id) {
            wheres.push('(mandals = ?)');
...
app.post(['/dashboardapi/postbusiness', '/api/postbusiness'], async (req, res) => {
    try {
        const b = req.body;
        await db.query(`INSERT INTO business (type, title, description, number, image, address, denomation, districtname, constituencyname, mandals, village_name, ministry_id, ward, usr_id, name)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
...
app.all(['/dashboardapi/getbusiness', '/dashboardapi/searchingbusiness', '/dashboardapi/searchingdemonation', '/api/getbusiness'], async (req, res) => {
    try {
        ...
        if (denomation) {
            wheres.push('denomation = ?');
            params.push(denomation);
        }
        if (title) {
...
app.post(['/dashboardapi/postmarriages', '/dashboardapi/postingmarriages', '/api/postmarriages'], async (req, res) => {
    try {
        const m = req.body;
        await db.query(`INSERT INTO marriages (gender, status, name, denomation_id, ministry_id, believer_id, pastor_id, image, dob, work, location, address, description, phonenumber, districtname, constituencyname, mandals, village_name, height, color, whealth, types, self, caste, subcaste, spirti, usr_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
...
app.all(['/dashboardapi/searchmarriages', '/dashboardapi/Searchmarriages', '/dashboardapi/searchingmarriages', '/api/getmarriages'], async (req, res) => {
    try {
        ...
        if (denomation_id) {
            wheres.push('denomation_id = ?');
            params.push(denomation_id);
        }
        if (district_id) {
            wheres.push('(districtname = ? OR id = ?)');
            params.push(district_id, district_id);
...
// Church Timings
app.post(['/dashboardapi/postchuechmeetings', '/api/postchurchtimings'], async (req, res) => {
    try {
        const c = req.body;
        await db.query(`INSERT INTO church_timings (church, service_name, day, time_start, time_end, typetime, description, district_id, constituency_id, mandal_id, village_id, denomationid, ministry_id, address, location, image, number, usr_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
...
app.all(['/dashboardapi/getchurches', '/dashboardapi/searchingchurchdata', '/api/getchurches'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM church_timings ORDER BY id DESC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.post(['/dashboardapi/postmeetings', '/api/postmeetings'], async (req, res) => {
    try {
        const m = req.body;
        await db.query(`INSERT INTO meetings (mettingtype, speakerone, speakertwo, speakerthree, speakerfour, fromdate, todate, fromtime, totime, image, districtname, constituencyname, mandals, village_name, description, address, location, facebook, youtube, denomation, usr_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
...
const getMeetingHandler = (type) => async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM meetings WHERE LOWER(mettingtype) LIKE ? ORDER BY id DESC', [`%${type}%`]);
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.post(['/dashboardapi/postadds', '/api/postadds'], async (req, res) => {
    try {
        const a = req.body;
        await db.query('INSERT INTO ads (title, type, image, description, number, usr_id) VALUES (?, ?, ?, ?, ?, ?)', [a.title || '', a.type || '', a.image || (Array.isArray(a.reviewImg) ? JSON.stringify(a.reviewImg) : null), a.description || '', a.number || '', a.usr_id || '']);
...
app.post(['/dashboardapi/postnews', '/api/postnews'], async (req, res) => {
    try {
        const n = req.body;
        await db.query('INSERT INTO news (title, description, content, image, category, posted_date) VALUES (?, ?, ?, ?, ?, ?)', [n.title || '', n.description || '', n.content || '', n.image || null, n.category || 'General', new Date()]);
...
app.post(['/dashboardapi/posthelping', '/api/posthelping'], async (req, res) => {
    try {
        const h = req.body;
        await db.query('INSERT INTO help_requests (name, phone, email, help_type, description, address) VALUES (?, ?, ?, ?, ?, ?)', [h.name || '', h.phone || '', h.email || null, h.help_type || '', h.description || '', h.address || '']);
...
app.all(['/dashboardapi/gethelps', '/api/gethelps'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM help_requests ORDER BY id DESC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.post(['/dashboardapi/contact', '/api/contact'], async (req, res) => {
    try {
        const c = req.body;
        await db.query('INSERT INTO contacts (name, email, phone, subject, message) VALUES (?, ?, ?, ?, ?)', [c.name || '', c.email || '', c.phone || '', c.subject || '', c.message || '']);
...
app.post(['/dashboardapi/passwordwebsitelogin', '/api/login'], async (req, res) => {
    try {
        ...
        const [users] = await db.query('SELECT id, name, mobile_number, email, category FROM users WHERE mobile_number = ? AND password = ?', [mobile_number, password]);
        if (!users || users.length === 0) {
            const [byPhone] = await db.query('SELECT id FROM users WHERE mobile_number = ?', [mobile_number]);
...
app.post(['/dashboardapi/postwebsitesignup', '/api/signup'], async (req, res) => {
    try {
        ...
        const [existing] = await db.query('SELECT id FROM users WHERE mobile_number = ?', [phone]);
        if (existing && existing.length > 0) {
            return res.json({ status: 300, message: 'Account with this phone number already exists!' });
        }
        const [resHeader] = await db.query('INSERT INTO users (name, mobile_number, email, password, category) VALUES (?, ?, ?, ?, ?)', [name, phone, userMail, password, Number(category) || 1]);
...
app.all(['/dashboardapi/getdistricts', '/dashboardapi/getmdistricts', '/api/districts', '/districts'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, distrct_nm, distrct_nm as districtname FROM districts ORDER BY distrct_nm ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getconsistencys', '/dashboardapi/getmconsistencys', '/api/constituencies', '/constituencies'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, const_nm, const_nm as constituencyname, dstrct_id FROM constituencies ORDER BY const_nm ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getmandals', '/dashboardapi/getmmandals', '/api/mandals', '/mandals'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, mndl_nm, mndl_nm as mandalname, const_id, dstrct_id FROM mandals ORDER BY mndl_nm ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/gepanchayati', '/dashboardapi/gempanchayati', '/api/panchayats', '/panchayats'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, pnchyt_nm, pnchyt_nm as panchayatname, mndl_id, const_id FROM panchayats ORDER BY pnchyt_nm ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/denomations', '/api/denominations', '/denominations'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, denomation_name, denomation_name as denomination_name FROM denominations ORDER BY id ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/leaderlevels', '/api/leaderlevels', '/leaderlevels'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, level_name, level_name as level FROM leader_levels ORDER BY id ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/educationalq', '/api/educationalq', '/educationalq'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, qualification_name, qualification_name as name, qualification_name as qualification FROM educational_qualifications ORDER BY id ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getwing', '/api/wings', '/wings'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, name, name as position, name as wingname, description FROM wings ORDER BY id ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getservices', '/api/services', '/services'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, service_name, service_name as servicename FROM services_list ORDER BY id ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/addNew', '/api/addNew', '/addNew'], async (req, res) => {
    try {
        ...
        const [existing] = await db.query('SELECT id FROM services_list WHERE service_name = ?', [serviceName]);
        if (existing && existing.length > 0) {
...
app.all(['/dashboardapi/pattern', '/api/pattern', '/pattern'], async (_req, res) => {
    res.json({
        status: 200,
        data: [
...
app.all(['/dashboardapi/getbelivers', '/dashboardapi/getbeliversdata', '/api/believers'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, fname, lname, CONCAT(fname, " ", COALESCE(lname, "")) as name, mobile_number, email FROM believers ORDER BY id DESC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getpastor', '/api/pastors'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, name, name as pastorname, mobile_number, email, denomination, church_name, district, constituency, mandal FROM pastors ORDER BY id DESC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getpastorsfilters', '/api/getpastorsfilters'], async (req, res) => {
    try {
        ...
        const wheres = [];
        if (districts) {
            wheres.push('(district = ? OR id = ?)');
            params.push(districts, districts);
        }
        if (constituencyname) {
            wheres.push('constituency = ?');
            params.push(constituencyname);
        }
        if (mandal_id) {
            wheres.push('mandal = ?');
            params.push(mandal_id);
...
app.all(['/dashboardapi/getchurch', '/api/getchurch'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, church_name, pastor_name, mobile_number, email, denomination, district, constituency, mandal, address FROM churches ORDER BY id DESC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getpastorassociation', '/dashboardapi/getpastorassociations', '/dashboardapi/getpastorassci', '/api/pastorassociations'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, association_name, association_name as pa_name, leader_name, mobile_number, email, district, constituency FROM pastor_associations ORDER BY id DESC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getinstitutes', '/dashboardapi/Searchinstitute', '/api/institutes'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT id, institutename, institutename as name, courses, location, phonenumber, description FROM institutes ORDER BY id DESC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.all(['/dashboardapi/getorganizations', '/dashboardapi/searchorganization', '/dashboardapi/searchinorganizations', '/api/organizations'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM organisations ORDER BY id DESC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
app.post(['/dashboardapi/postindepedentchurch', '/api/postindepedentchurch'], async (req, res) => {
    try {
        const c = req.body;
        const name = c.churchname || c.church_name || 'Independent Church';
        const phone = c.contactnumber || c.mobile_number || c.phone;
        await db.query('INSERT INTO churches (church_name, pastor_name, mobile_number, email, denomination, district, constituency, mandal, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, c.pastor_id || '', phone || '', c.email || null, c.denomination_id || '', c.district_id || '', c.constituency_id || '', c.mandal_id || '', c.street || c.address || '']);
        await db.query('INSERT INTO registrations (registration_type, first_name, last_name, email, phone, additional_data) VALUES (?, ?, ?, ?, ?, ?)', ['independentchurch', name, '', c.email || null, phone || null, JSON.stringify(c)]);
...
// Wish Form
app.post(['/dashboardapi/postwishform', '/api/postwishform'], async (_req, res) => {
...
app.post(['/dashboardapi/postinsututies', '/api/postinstitutes'], async (req, res) => {
    try {
        const i = req.body;
        await db.query('INSERT INTO institutes (institutename, collegetype, courses, phonenumber, location, address, description, district_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [i.institutename || i.name || '', i.collegetype || '', i.courses || '', i.phonenumber || i.number || '', i.location || '', i.address || '', i.description || '', i.district_id || '']);
...
app.post(['/dashboardapi/postcolleges', '/api/postcolleges'], async (req, res) => {
    try {
        const c = req.body;
        await db.query('INSERT INTO colleges (collegename, location, address, phonenumber, districtname, constituencyname, mandals) VALUES (?, ?, ?, ?, ?, ?, ?)', [c.collegename || '', c.location || '', c.address || '', c.phonenumber || '', c.districtname || '', c.constituencyname || '', c.mandals || '']);
...
app.post(['/dashboardapi/checknumber', '/api/checknumber'], async (req, res) => {
    try {
        ...
        const [rows] = await db.query('SELECT id FROM users WHERE mobile_number = ?', [phone]);
        if (!rows || rows.length === 0) {
...
app.all(['/dashboardapi/getadocumentsdataa', '/api/downloads'], async (_req, res) => {
    try {
...
app.all(['/dashboardapi/getLeaderswebsiteData', '/dashboardapi/getLeaderswebsitewing', '/api/leaders'], async (_req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM leaders ORDER BY id ASC');
        res.json({ status: 200, data: rows });
    }
    catch (err) {
        res.status(500).json({ status: 500, error: err.message });
...
// -------------------------------------------------------------
app.get(['/api/admin-database-view', '/admin-database-view', '/admin'], async (_req, res) => {
    try {
        const [tables] = await db.query('SHOW TABLES');
        const tableSummaries = [];
        for (const t of tables) {
            const name = Object.values(t)[0];
            const [cnt] = await db.query(`SELECT COUNT(*) as count FROM \`${name}\``);
            const count = cnt && cnt[0] ? Number(cnt[0].count) : 0;
  ...
  <script>
    ...
    window.onload = () => {
      loadTable(currentTable);
        ...
        res.status(500).send(`<h2>Database Connection Error</h2><p>${err.message}</p>`);
    }
});
// Root ping
